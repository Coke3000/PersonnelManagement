package com.wyu.controller;

import com.alibaba.fastjson.JSON;
import com.github.pagehelper.PageHelper;
import com.wyu.domain.Dept;
import com.wyu.domain.Job;
import com.wyu.domain.User;
import com.wyu.service.JobService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;
import java.io.IOException;
import java.util.List;

@RestController
public class JobController {
    @Autowired
    private JobService js;
    //职位查询
    @RequestMapping("FindJob.do")
    public void findjob(int  page, int  limit, HttpServletResponse response) throws IOException {
//处理将数据返回前端页面出现中文乱码问题
        response.setContentType("text/html;charset=utf-8");
        System.out.println("当前页："+page);
        System.out.println("条数："+limit);
        PageHelper.startPage(page,limit);
        List<Job> list=js.findJob();
        int total=js.getTotal();
        System.out.println("总记录："+total);
        //用fastjson工具将list集合构建json数据
        String json = JSON.toJSONString(list);
        response.getWriter().write("{\"code\":0,\"msg\":\"\",\"count\":"+total+",\"data\":"+json+"}");
    }

    //职位查询编辑功能
    @RequestMapping("/UpdateJob.do")
    public String updatejob(Job job, HttpSession session){
        String msg="false";
        //获得session域中的user
        User user2 = (User) session.getAttribute("user");
        if(user2.getRoleId()==3){
            //System.out.println("要修改的信息:"+user);
            int i=js.updateJobByjid(job);
            if(i>0){
                msg="true";
            }else{
                msg="false";
            }
        }
        return msg;
    }

    //职位查询删除功能
    @RequestMapping("/DeleteJob.do")
    public String deletejob(String jname,HttpSession session){
        String msg="false";
        //获得session域中的user
        User user2 = (User) session.getAttribute("user");
        if(user2.getRoleId()==3){
            int i=js.deleteJobByjname(jname);
            if(i>0){
                msg="true";
            }else{
                msg="false";
            }
        }
        return msg;
    }


    //职位查询搜索功能
    @RequestMapping("/SearchJob.do")
    private void searchuser(int page,int limit, Job job, HttpServletResponse response) throws IOException {
        //处理将数据返回前端页面出现中文乱码问题
        response.setContentType("text/html;charset=utf-8");
        PageHelper.startPage(page,limit);
        System.out.println("要查询的数据："+job);
        List<Job> list=js.searchJob(job);
        //得到总记录数
        int total=js.getTotal();
        System.out.println("总记录："+total);
        //用fastjson工具将list集合构建json数据
        String json = JSON.toJSONString(list);
        response.getWriter().write("{\"code\":0,\"msg\":\"\",\"count\":"+total+",\"data\":"+json+"}");
    }

    //职位添加
    @RequestMapping("/AddJob.do")
    public String addJob(Job job,HttpSession session){
        String msg="false";
        User user2 = (User) session.getAttribute("user");
        if(user2.getRoleId()==3||user2.getRoleId()==2){
            int i=js.addJob(job);
            if(i>0){
                msg="true";
            }else {
                msg="false";
            }
        }
        return msg;
    }


}
